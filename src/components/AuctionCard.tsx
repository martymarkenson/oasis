...
export default function AuctionCard({ auction }: AuctionCardProps) {
  ...
  const [liveData, setLiveData] = useState({ currentBid: auction.currentBid, totalBids: auction.totalBids });

  useEffect(() => {
    const updateLiveData = async () => {
      if (auction.auctionLink) {
        const scrapedData = await ScrapingService.scrapeAuctionData(auction.auctionLink);
        setLiveData(scrapedData);
      }
    };
      ...
      <div className="p-4 space-y-3 flex-1 flex flex-col">
        ...
        <div className="space-y-3 flex-1">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-bold text-black">${liveData.currentBid?.toLocaleString() || 0}</span>
          </div>
          ...
          <div className="flex items-center justify-between text-gray-600">
            ...
            <span className="font-medium">{liveData.totalBids}</span>
          </div>
